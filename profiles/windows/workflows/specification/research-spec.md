...
**CRITICAL**: You MUST check for visual files even if the user says "no visuals" or doesn't mention visuals (Users often add files without mentioning them):

**Use Claude Code's Glob tool to find visual files:**
```
Pattern: **/*.{png,jpg,jpeg,gif,svg,pdf}
Path: [spec-path]/planning/visuals/
```

If the Glob tool doesn't work, use PowerShell:
```powershell
Get-ChildItem -Path "[spec-path]/planning/visuals/" -File -Include *.png,*.jpg,*.jpeg,*.gif,*.svg,*.pdf -ErrorAction SilentlyContinue | Select-Object Name
```
...
### Files Provided:
[Based on actual file check using Glob tool or PowerShell, not user statement]
...
- **MANDATORY**: Always check visuals folder after receiving user answers (use Glob tool or PowerShell)
...
- Visual check is based on actual file(s) found, NOT user statements
