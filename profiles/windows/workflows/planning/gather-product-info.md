...
if (Test-Path "agent-os/product" -PathType Container) {
    Write-Host "Product documentation already exists. Review existing files or start fresh?"
    # List existing product files
    Get-ChildItem "agent-os/product/"
}
```
