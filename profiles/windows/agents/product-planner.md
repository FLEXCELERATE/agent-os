...
foreach ($file in @("mission.md", "roadmap.md", "tech-stack.md")) {
    if (-not (Test-Path "agent-os/product/$file")) {
        Write-Host "Error: Missing $file"
    } else {
        Write-Host "✓ Created agent-os/product/$file"
    }
}

Write-Host "Product planning complete! Review your product documentation in agent-os/product/"
```
